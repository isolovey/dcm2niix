...
void showHelp(const char * argv[], struct TDCMopts opts) {
    ...
    printf("  -e : export as NRRD instead of NIfTI (y/n, default n)\n");
     ...
     #define kQstr ""
    #endif
    printf("  -f : filename (%%a=antenna (coil) name, %%b=basename, %%c=comments, %%d=description, %%e=echo number, %%f=folder name, %%i=ID of patient, %%j=seriesInstanceUID, %%k=studyInstanceUID, %%m=manufacturer, %%n=name of patient, %%p=protocol,%s %%r=instance number, %%s=series number, %%t=time, %%u=acquisition number, %%v=vendor, %%w=frame label, %%x=study ID; %%z=sequence name; default '%s')\n", kQstr, opts.filename);
